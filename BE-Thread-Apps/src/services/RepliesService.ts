...
export default new (class RepliesService {
  ...
  async InsertReplies(data: object): Promise<object> {
    try {
      await this.RepliesRepository.createQueryBuilder()
        .insert()
        .into(Replies)
        .values(data)
        .execute();
  ...
  async getReplies(): Promise<object> {
    try {
      const dataReplies = await this.RepliesRepository.createQueryBuilder(
        "replies"
      )
  ...
  async getReplyByThreads(id: number): Promise<object> {
    try {
      const getUser = await this.RepliesRepository.createQueryBuilder("replies")
        .leftJoinAndSelect("replies.threads", "threads")
        .leftJoinAndSelect("replies.users", "users")
        .where("replies.threads = :id ", { id })
        .getMany();
  ...
  async updateReplies(data: object, id: number): Promise<object> {
    try {
      const updateData = await this.RepliesRepository.createQueryBuilder()
        .update(Replies)
        .set(data)
        .where({ id })
        .execute();
  ...
  async deleteReplies(id: number): Promise<object> {
    try {
      const deleteData = await this.RepliesRepository.createQueryBuilder()
        .delete()
        .from(Replies)
        .where({ id })
        .execute();
